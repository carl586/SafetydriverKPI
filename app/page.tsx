...
export default function DashboardPage() {
  ...
  // Companies derived from whatever is actually in the drivers data —
  // add a driver with a new company value and it shows up here automatically.
  const companies = Array.from(new Set(drivers.map((d) => d.company).filter(Boolean))).sort();

  // Company-scoped views. Everything below reads from these instead of the raw arrays.
  const scopedDrivers = selectedCompany === 'all' ? drivers : drivers.filter((d) => d.company === selectedCompany);
  const scopedEvents = selectedCompany === 'all' ? events : events.filter((e) => e.company === selectedCompany);

  const filteredEvents = () => {
    if (currentSubTab === 0) return scopedEvents;
    if (currentSubTab === 1) return scopedEvents.filter((e) => e.event_type === 'Violation');
    if (currentSubTab === 2) return scopedEvents.filter((e) => e.event_type === 'Inspection');
    if (currentSubTab === 3) return scopedEvents.filter((e) => e.event_type === 'Accident');
    if (currentSubTab === 4) return scopedEvents.filter((e) => e.event_type === 'Samsara Event');
    return scopedEvents;
  };
  ...
  const grandTotalPoints = scopedDrivers.reduce((sum, d) => sum + getTotalPoints(d), 0);
  const totalViolations = scopedEvents.filter((e) => e.event_type === 'Violation' || e.event_type === 'Inspection').length;
  const totalAccidents = scopedEvents.filter((e) => e.event_type === 'Accident').length;
        ...
        <div className="max-w-7xl mx-auto px-6 py-5 flex items-center justify-between">
          ...
          <div className="flex gap-8 text-sm items-center">
            ...
            <select
              ...
              className="border rounded-xl px-3 py-2 text-sm font-medium ml-4"
            >
      ...
      {currentTab === 2 && (
        <div className="max-w-7xl mx-auto px-6 py-8">
          <div className="flex justify-between items-center mb-8">
            ...
            <button
              onClick={() => {
                setNewDriver((f) => ({ ...f, company: selectedCompany === 'all' ? f.company : selectedCompany }));
